// ============================================================
// Category Controller - HTTP adapter for category endpoints
...
export class CategoryController {
  static async list(request: NextRequest) {
    try {
      ...
      const result = await useCases.getCategories.execute(
        {
  ...
  static async delete(_request: NextRequest, id: string) {
    try {
      await useCases.deleteCategory.execute(id, _request)
      return ResponsePresenter.success({ message: 'Categoría desactivada exitosamente' })
    } catch (error) {
      return ResponsePresenter.error(error)
    }
  }
