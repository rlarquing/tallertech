...
async function fetchExportData(entity: string, params: ExportParams) {
  ...
  if (params.dateFrom || params.dateTo) {
    ...
    if (params.dateFrom) (dateFilter.createdAt as Record<string, unknown>).gte = new Date(params.dateFrom)
    if (params.dateTo) (dateFilter.createdAt as Record<string, unknown>).lte = new Date(params.dateTo)
  }
...
async function generateXLSX(data: Record<string, unknown>[], entity: string): Promise<Buffer> {
...
async function generatePDF(data: Record<string, unknown>[], entity: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.text(`Generado: ${new Date().toLocaleDateString('es-BO', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`, { align: 'center' })
    doc.moveDown(1.5)
    ...
    const maxRowsPerPage = Math.floor((doc.page.height - y - 60) / rowHeight)
    ...
    while (rowIndex < data.length) {
      ...
      for (const label of headerLabels) {
        ...
        doc.text(truncatedLabel, x + 3, y + 7, { width: colWidth - 6, lineBreak: false })
        x += colWidth
      }
      y += headerHeight
      ...
      for (let i = 0; i < pageRows; i++) {
        ...
        for (const header of headers) {
          ...
          doc.text(truncated, x + 3, y + 5, { width: colWidth - 6, lineBreak: false })
          x += colWidth
        }
        y += rowHeight
        rowIndex++
      ...
      doc.text(`Página ${pageNum} | ${data.length} registros`, startX, doc.page.height - 40, { width: pageWidth, align: 'center' })
...
export async function exportData(params: ExportParams): Promise<{
  ...
  const data = await fetchExportData(params.entity, params) as Record<string, unknown>[]
